...
public final class EvgenAnalytics {
     ...
     */
    public func caseDemoSampleEvent(userSessionId: String = "guest", screenTitle: String) {
        var options: [String: Any] = [:]
        options["USER_SESSION_ID"] = userSessionId
        options["SCREEN_TITLE"] = screenTitle
        let interfacesDict: [String: Any] = [:]
        let _meta = makeMeta(1, interfaces: interfacesDict)
        options["_meta"] = _meta
        trackEvent("CaseDemo.SampleEvent", withOptions: options)
    }
