...
code:
  ...
  ConfigName1:
    ...
    param_name_case: string    # Стиль именования параметров (опционально): snake | camel | pascal | upper | no (дефолт для type_script — camel, для kotlin/java/swift/dart/c_sharp — no; см. хелпер `paramNameCase` в шаблонах)
...
Эта документация охватывает все основные возможности EvGen. Для более детального изучения рекомендуется изучить примеры в папке `tutorial/` пакета. Про `param_name_case` на всех языках генерации — `tutorial/10.param_name_case/` (в конфиге задано `param_name_case: upper`).
